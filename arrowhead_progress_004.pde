...
float len = 15; // length of each step
float angle = 60;
...
void draw() {
  ...
  plot(7); // which order to plot
  save("arrowhead-progress-004-tn.png");
 // exit();
}
...
void plot(int order) {
  ...
  if (order == 7) {
    println("Plotting 6th Order");
    int lenOfSeventhOrder = seventhOrder.size() /2;
    for (int i = 0; i < lenOfSeventhOrder; i = i+1) {
      String item = seventhOrder.get(i);
      if (item == "+") {
        rotateTurtle(angle);
  ...
  //---  Start of 9th order
  if (order == 9) {
    int lenOfNinthOrder = ninthOrder.size() /2;
